...
app.config["DOWNLOADED_DIR"] = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "../downloaded"
)
...
class DownloadForm(FlaskForm):
    ...
    submit = SubmitField(
        "Stáhnout videa", render_kw={"class": "btn btn-primary btn-lg"}
    )
    ...
    for item in to_download:
        ...
        with youtube_dl.YoutubeDL(ydl_opts) as ydl:
            ...
            try:
                ...
                cur.execute(
                    "UPDATE urls SET state=1, errorlog=? WHERE id=?",
                    (logger.get_log(), item["id"]),
                )
                continue
...
def list():
    cur = get_db().cursor()
    cur.execute(
        "SELECT id,url,audio,state,created_at, (SELECT group_concat(filename, ';') FROM files WHERE urls.id=files.url) AS files FROM urls ORDER BY id DESC"
    )
    rows = cur.fetchall()
...
def redownload(urlid):
    ...
    cur = con.cursor()
    cur.execute(
        "INSERT INTO urls (url, audio) SELECT url, audio FROM urls WHERE id=?", (urlid,)
    )
    con.commit()
    run_downloader()
